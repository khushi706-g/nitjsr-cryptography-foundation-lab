...
def tripleDESEncryption(plaintext, key1, key2, key3):
    return DES.desEncryption(
        DES.desDecryption(DES.desEncryption(plaintext, key1), key2), key3
    )


def tripleDESDecryption(ciphertext, key1, key2, key3):
    return DES.desDecryption(
        DES.desEncryption(DES.desDecryption(ciphertext, key3), key2), key1
    )
...
if __name__ == "__main__":
    ...
    print(
        "Plaintext:",
        DES.binToHex(tripleDESDecryption(ciphertext, key1, key2, key3)),
        "\n",
    )
